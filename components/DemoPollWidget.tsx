...
const DemoPollWidget: React.FC = () => {
  ...
  // Card wrapper styles
  const cardClass =
    "rounded-xl border border-jury-border bg-jury-surface shadow-[0_30px_70px_-30px_rgba(0,0,0,.9)] p-6 sm:p-8";
      ...
      <div className={cardClass}>
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <IconLoader2 className="w-8 h-8 animate-spin text-jury-emerald mx-auto mb-4" />
            <p className="text-jury-muted text-sm">
              Loading live poll...
            </p>
          </div>
        </div>
  ...
  return (
    <div className={cardClass}>
      ...
      <div className="text-center mb-6">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-jury-emerald-tint text-jury-emerald text-xs font-medium rounded-full mb-4">
          <span className="w-1.5 h-1.5 rounded-full bg-jury-emerald animate-pulse" />
          Live Poll
        </span>
        <h3 className="text-lg font-semibold text-jury-text mb-1">
          {demoPoll.question}
        </h3>
        {demoPoll.description && (
          <p className="text-sm text-jury-muted">
            {demoPoll.description}
          </p>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        {!hasVoted ? (
          ...
          >
            {options.map((option) => (
              <motion.button
                ...
                disabled={isVoting}
                className="w-full p-3.5 text-left rounded-xl border border-jury-border hover:border-jury-emerald-line hover:bg-jury-emerald-tint transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 rounded-full border-2 border-jury-dim group-hover:border-jury-emerald transition-colors shrink-0" />
                  <span className="font-medium text-jury-text text-sm">
                    {option.text}
                  </span>
                </div>
              </motion.button>
            ...
            {isVoting && (
              <div className="text-center text-jury-emerald text-sm font-medium pt-2">
          ...
          <motion.div
            ...
            className="space-y-3"
          >
            <div className="flex items-center justify-center gap-2 text-jury-emerald font-medium text-sm mb-4">
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{
              ...
              return (
                <motion.div
                  ...
                  className={`relative p-3 rounded-xl border overflow-hidden ${
                    isSelected
                      ? "border-jury-emerald-line bg-jury-emerald-tint"
                      : "border-jury-border"
                  }`}
                >
                  ...
                  <div className="relative flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      {isSelected && (
                        <IconCheck className="w-3.5 h-3.5 text-jury-emerald" />
                      )}
                      <span className="font-medium text-jury-text text-sm">
                        {result.option_text}
                      </span>
                    ...
                    <div className="text-right">
                      <span className="font-bold text-jury-text text-sm">
                        {percentage}%
                      </span>
                      <span className="text-xs text-jury-muted ml-1.5">
                        ({result.vote_count})
                      </span>
                    </div>
                  </div>
      ...
      <div className="text-center mt-5 pt-4 border-t border-jury-border">
        <p className="text-xs text-jury-muted">
          Updating in real time · {totalVotes} total
        </p>
      </div>
    </div>
  );
